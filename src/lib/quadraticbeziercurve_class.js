...
class QuadraticBezierCurveClass {
  constructor() {
    // 属性
  ...
  //完全相等，包括精度。
  ExactlyEqual(el) {
    if (((this.Point1.ExactlyEqual(el.Point1)) && (this.Point2.ExactlyEqual(el.Point2)) && (this.Point3.ExactlyEqual(el.Point3)) && this.FixedNumber == el.FixedNumber) || ((this.Point1.ExactlyEqual(el.Point2)) && (this.Point2.ExactlyEqual(el.Point1)) && this.FixedNumber == el.FixedNumber)) {
      return true;
    }
    return false;
  }
  ...
  //判断是否线类
  IsClass(e) {
    return e instanceof QuadraticBezierCurveClass;
  }
  ...
  GetSlope() {
    ...
    return parseFloat(((this.Point3.Y - this.Point1.Y) / (this.Point3.X - this.Point1.X)).toFixed(this.FixedNumber));
  }
  //获取角度
  GetAngle() {
    var idiff_x = this.Point3.X - this.Point1.X;
    var idiff_y = this.Point3.Y - this.Point1.Y;
    //返回角度,不是弧度
    if (idiff_x != 0) {
...
module.exports = QuadraticBezierCurveClass;