const Class = require('class-extender');
...
class FontsClass {
      ...
      if (arguments.length == 2) {
        ...
        if (_.isArray(arguments[0])) {
          if (arguments[0].length > 0) {
            _.each(arguments[0], function (ele, index) {
  ...
  //生成一个克隆体
  Clone() {
    var aNewClass = new RegularPolygonClass(this);
    return aNewClass;
  }
  Contains(epoint) {
  ...
  //判断是否线类
  IsClass(e) {
    return e instanceof RegularPolygonClass;
  }
